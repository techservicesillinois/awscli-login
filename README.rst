...
password
    ...
    supplied the user will be prompted. It is not recommended to use
    this property. Instead supply the password manually or use the
...
duration
    ...
    for details)::
        ...
        duration = 3600
http_header_factor
    Name of HTTP Header to store the user's Duo factor in::

        http_header_factor = X-Shibboleth-Duo-Factor
http_header_passcode
    Name of HTTP Header to store the user's passcode in::
...
example the property `ecp_endpoint_url` becomes `--ecp_endpoint_url`.
For details on these option see the documentation above or refer
to the online documentation.  Below only those options not avaliable
as properties are documented below.
...
If you receive a bad interpreter error from the WSL it may be because
the location where the awscli is installed is not listed before the
location of the Windows install of awscli::
